...
class TableProvider extends ChangeNotifier {
  ...
  void setMonedaForTable(int table, String moneda) {
    final items = _tables[table];
    if (items == null) return;
    _tables[table] = items.map((i) => CartItem(
      id: i.id,
      ...
      moneda: moneda,
      cantidad: i.cantidad,
      subtotalFiat: i.subtotalFiat,
      subtotalSats: i.subtotalSats,
    )).toList();
  }
  ...
  Future<void> saveCartToTable(int table, List<CartItem> items, String moneda) async {
    _tables[table] = items;
    setMonedaForTable(table, moneda);
    await _persist();
    notifyListeners();
  }
  ...
  Future<void> closeCurrentTable() async {
    if (_currentTable != null) {
      await closeTable(_currentTable!);
    }
  }
